...
p_c = scCustomize::FeaturePlot_scCustom(BuenColors::shuf(seu), features = "young_cohort_frac", pt.size = 0.05, order = F) +
  coord_fixed() +
  ggtitle("Fraction of cells from Young donors,\nper cluster") +
  theme_pubr(base_size = 5) +
...
save_plot("Suppl.Fig.3.pdf", p_combined, base_height = 200, base_width = 180, units="mm")
...
p = scCustomize::FeaturePlot_scCustom(seu, features = "young_cohort_frac", pt.size = 0.05) +
  coord_fixed() +
  ggtitle("Fraction of cells from Young donors, per cluster") +
  theme_pubr(base_size = 5) +
  theme(axis.line = element_line(linewidth=0.4, color="black"),
...
save_plot("UMAP_colored_by_young_fraction.pdf", p, base_height = 72.72, base_width = 72.72, units="mm")
...
p
save_plot("Suppl.Fig.6.pdf", p, base_height = 200, base_width = 180, units="mm")
